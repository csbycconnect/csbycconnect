...
export default function Blogs() {
    ...
    // Unique authors + categories derived from posts
    ...
    const filtered = useMemo(() => {
        ...
        if (selectedReadTime !== null) {
            const range = READ_TIME_RANGES[selectedReadTime];
            posts = posts.filter(p => range.fn(p.readTime));
        }
    ...
    const hasFilters = search || selectedAuthor || selectedCategory || selectedReadTime !== null || sortBy !== 'recent';
            ...
            <main className="blog-page-container">
                ...
                <div className="blog-filter-section">
                    ...
                    <div className="blog-filter-group">
                        <span className="blog-filter-label">Author</span>
                        <div className="blog-filter-chips">
                            {authors.map(a => (
                                <button
                                    key={a}
                                    className={`blog-filter-chip ${selectedAuthor === a ? 'active' : ''}`}
                                    onClick={() => setSelectedAuthor(selectedAuthor === a ? null : a)}
                                >
                                    {a}
                                </button>
                            ))}
                    ...
                    <div className="blog-filter-group">
                        ...
                        <div className="blog-filter-chips">
                            {categories.map(c => (
                                <button
                                    key={c}
                                    className={`blog-filter-chip ${selectedCategory === c ? 'active' : ''}`}
                    ...
                    {/* Read time chips */}
                    <div className="blog-filter-group">
                        <span className="blog-filter-label">Read Time</span>
                        <div className="blog-filter-chips">
                            {READ_TIME_RANGES.map((r, i) => (
                                <button
                                    key={r.label}
                                    className={`blog-filter-chip ${selectedReadTime === i ? 'active' : ''}`}
                                    onClick={() => setSelectedReadTime(selectedReadTime === i ? null : i)}
                                >
                                    {r.label}
                                </button>
                            ))}
                    ...
                    <div className="blog-filter-group">
                        ...
                        <div className="blog-filter-chips">
                            {SORT_OPTIONS.map(s => (
                                <button
                                    key={s.key}
                                    className={`blog-filter-chip ${sortBy === s.key ? 'active' : ''}`}
                    ...
                    {/* Clear all */}
                    {hasFilters && (
                        <button className="blog-filter-clear-all" onClick={clearAll}>
                            ✕ Clear All
                        </button>
                    )}
                </div>
                ...
                {filtered.length === 0 ? (
                    <div className="blog-no-results">
                        ...
                        <button className="blog-filter-clear-all" onClick={clearAll} style={{ marginTop: '1rem' }}>Clear Filters</button>
                    </div>
                ) : (
                    <AnimatedList
                        items={filtered}
